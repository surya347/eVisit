...
History:
 ...
 Action:Created
******************************************************************************/
trigger CK_OpportunityTrigger on Opportunity (
	before insert, 
	before update, 
	before delete, 
	after insert, 
	...
	after undelete) {
		...
		if(settings.containsKey(String.valueOf(Opportunity.SobjectType))){
			if(settings.get(String.valueOf(Opportunity.SobjectType)).Track_Changes__c){
			...
			if(settings.get(String.valueOf(Opportunity.SobjectType)).Sync_Deletes_Undeletes__c){
