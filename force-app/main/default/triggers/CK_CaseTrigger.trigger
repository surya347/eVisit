...
Description:Trigger to handle all Case Events
...
History:
 ...
 Action:Created
******************************************************************************/
trigger CK_CaseTrigger on Case (
	before insert, 
	before update, 
	before delete, 
	after insert, 
	...
	after undelete) {
		...
		if(settings.containsKey(String.valueOf(Case.SobjectType))){
			if(settings.get(String.valueOf(Case.SobjectType)).Track_Changes__c){
			...
			if(settings.get(String.valueOf(Case.SobjectType)).Sync_Deletes_Undeletes__c){
